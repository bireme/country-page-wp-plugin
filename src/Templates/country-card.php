...
<div class="cp-card">
  ...
  $cp_has_body = $cp_show_title || $cp_show_excerpt || $cp_show_content
    || ($cp_show_meta && !empty($displayableAcfFields));
  ?>
  <?php if ($cp_has_body): ?>
  <div class="cp-card__body">
    <?php if ($cp_show_title || !empty($tags) || ($cp_show_excerpt && $excerpt !== '')): ?>
      <header class="cp-card__intro">
        <?php if ($cp_display_mode === 'default'): ?>
        <p class="cp-card__eyebrow"><?php esc_html_e('Perfil do Pais', 'country-pages'); ?></p>
        <?php endif; ?>
